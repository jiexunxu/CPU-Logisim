This project implements the multicycle CPU. 
...
Both programs are adapted from this assignment's original test programs.

Program 1: The Sum Program 

This program adds the numbers between A and B, inclusive, where A originally resides at address 44 hex in data memory and B resides in address 48 hex in data memory. The result is placed 
at data memory location 4c hex. Since there is no halt instruction on the MIPS processor, the program, after computing the result, will just loop (until you turn off the clock).  
